...
void initOpenGl()
{
   ...
   for(int i=0; i<2; i++)
   {
      ...
      glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, pos_loc, vbo[i]);
      glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, vel_loc, vbo[i]);
      glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, age_loc, vbo[i]);
